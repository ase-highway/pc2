...
function fullSlide(id, opt){
  ...
  // }
  // if($(id).hasClass('product-slider-list') && $(id + ' .swiper-slide').length > 5 && $(id + ' swiper-slide').length <= 15) {
  //   $(id + ' .swiper-wrapper').append($(id + ' .swiper-wrapper').html())
  //   $(id + ' .swiper-wrapper').append($(id + ' .swiper-wrapper').html())

  //   setTimeout(function(){
  //     if($(id).hasClass('product-slider-list')) $(id + " .swiper-button-prev").click();
  //   }, 100);
  // }
...
function popTooltip(){
  ...
  $(".pop-tooltip .close").on("click", function(){
    $tooltip = $(this).closest('.pop-tooltip');
    $tooltip.addClass('off');
    setTimeout(function(){
      $tooltip.removeClass('off');
      $tooltip.removeClass('on');
...
function reviewRecommend(){
  $('.product-review .btn-recommend').on('click', function(){
    $(this).toggleClass('on');
  });
}
...
function todayTableCategory(){
  $('.product-category li a, .product-category-grid li a').on('click', function(e){
    e.preventDefault();
    $(this).parent().addClass('on').siblings().removeClass('on');
  });
}

function todayTableDetailTab(){
  $('.today-table-detail .tab-type1 li a').on('click', function(e){
    e.preventDefault();
    $(this).parent().addClass('on').siblings().removeClass('on');
  });
}

function saleCategoryTab(){
  var $wrap = $('.sale-category-wrap');
  if(!$wrap.length) return;

  $wrap.find('.sale-category-box > ul > li a').on('click', function(e){
    e.preventDefault();
    var $li = $(this).parent();
    var idx = $li.index();
    $li.addClass('on').siblings().removeClass('on');
    $wrap.find('.sale-sub-category .sub-list').removeClass('on');
    $wrap.find('.sale-sub-category .sub-list[data-sub-index="' + idx + '"]').addClass('on');
  });

  $wrap.find('.sale-sub-category .sub-list li a').on('click', function(e){
    e.preventDefault();
    $(this).parent().addClass('on').siblings().removeClass('on');
  });
}
...
function orderDetailToggle(){
  $(document).on('click', '.table-type1 .order-row', function(e){
    if($(e.target).closest('.order-btns, .btn-order').length) return;
    var $row = $(this);
    var $detail = $row.next('.order-detail');
    $row.toggleClass('is-open');
    $detail.toggleClass('is-open');
  });

  $(document).on('click', '.table-type1 .order-btns, .table-type1 .btn-order', function(e){
    e.stopPropagation();
  });
}
